...
//!
//! Thin clap binary wired to `axi_core` through the `herdr` backend crate.
//! No CLI behavior, flags, or exit codes changed from the pre-refactor
//! single-crate layout.

use clap::{Parser, Subcommand};
use herdr::{dispatch, live_agents, wait as herdr_wait};
...
fn print_err(e: axi_core::AxiError) -> i32 {
...
fn main() {
    ...
    let code = match cli.cmd {
        Cmd::Agents { json } => match live_agents() {
            Ok(agents) => {
                if json {
                    ...
                    print!("{}", axi_core::format_agents_table(&agents));
                }
                0
            }
            Err(e) => print_err(e),
        ...
        Cmd::Fleet { json } => match live_agents() {
            Ok(agents) => {
                if json {
                    let counts = axi_core::fleet_counts(&agents);
                    println!(
                        "{}",
                    ...
                    println!("{}", axi_core::format_fleet(&agents));
                }
                0
            }
            Err(e) => print_err(e),
...
fn agents_json(agents: &[axi_core::Agent]) -> serde_json::Value {
    serde_json::json!(
        agents
            .iter()
